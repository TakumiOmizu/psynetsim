#' @title Simulating Dynamic Treatment Intervention (Plot Last Trial)
#' @description \code{simulate_network_threshold} simulates an intervention where the thresholds of targeted symptoms are directly manipulated. It returns full data history and plots the network structure and symptom trajectories from the **final trial** (not the average).
#'
...
#' @param target A numeric vector matching the number of symptoms. Values indicate the magnitude of intervention added to each symptom's threshold. Use 0 for no intervention.
...
#' \item{plot_data}{A dataframe for ggploting symptom trajectories.}
#' \item{raw_data}{A 3D array [Trial, Time, Symptom] containing raw activation values.}
#' \item{final_network}{A list containing the mean W and b at the final step.}
#' \item{network_history_full}{A nested list containing the full history of network structures for every trial and update.}
#' \item{result_plot}{A grid object displaying the network structure of the last trial and the global activation trajectory of the last trial.}
#' \item{result_text}{A summary string of the final activation level.}
#'
...
simulate_network_threshold <- function(W_init,
  ...
  simulate_network <- function(W_init, b_init, target, connectivity,
                               baseline_iteration, num_updates,
                               update_interval, follow_up_iteration, symptom_name) {
    ...
    # Intervention
    b <- b + target
    history_list[[2]] <- list(step_index = current_step, phase = "Post-Intervention", W = W, b = b)
  ...
  p2 <- tibble::tibble(time = 1:total_time, mean = last_trial_mean_traj, sd = last_trial_sd_traj) |>
    ...
    ggplot2::geom_vline(xintercept = baseline_iteration + 0.5, linetype="dashed", color="gray") +
